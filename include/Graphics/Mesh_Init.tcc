...
FUG_RESOURCE_INIT(Mesh, MeshInitInfo_Default) {
    ...
    if (!vertexData->usesTextureCoordinates()) {
        FUG_LOG(LogLevel::Error)("MeshInitInfo_Default: expected vertex data with texture coordinates\n");
        throw;
    }
    if (!vertexData->usesNormals()) {
        FUG_LOG(LogLevel::Error)("MeshInitInfo_Default: expected vertex data with normals\n");
        throw;
    }
    if (!vertexData->usesIndexing()) {
        FUG_LOG(LogLevel::Error)("MeshInitInfo_Default: expected vertex data with indexing\n");
        throw;
    }
