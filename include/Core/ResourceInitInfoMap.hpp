...
#define FUG_RESOURCE_INITINFO_INIT(T_Resource, T_InitInfo) \
    ...
    static fug::ResourceInitInfoMapEntry resourceInitInfoMap_##T_InitInfo( \
        #T_InitInfo, \
        addInitInfo<T_Resource, T_InitInfo>); \
    static void initInitInfo(const json& json, T_InitInfo& initInfo) \
...
namespace fug {
    ...
    public:
        void addInitInfo(std::string initInfoName,
                         std::function<void(const json&)> func);

        std::function<void(const json&)> getInitFunc(
    ...
    private:
        std::unordered_map<std::string,
                           std::function<void(const json&)>> _map;
    };

    class ResourceInitInfoMapEntry {
    public:
        ResourceInitInfoMapEntry(
            const std::string& initInfoName,
            std::function<void(const json&)> func);
    };
